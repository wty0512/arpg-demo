...
void GameMap::DrawMini(HDC hdcDest, const vector<Sprite *> & vecpSprite)
{
	...
	*/
	...
	::MoveToEx(hdcDest,0, GAME_HEIGHT-miniMap_Height-1, NULL);
	::LineTo(hdcDest, miniMap_Width +1,  GAME_HEIGHT-miniMap_Height-1);
	::LineTo(hdcDest, miniMap_Width +1, GAME_HEIGHT);

	BitBlt(hdcDest, 
			0, GAME_HEIGHT-miniMap_Height-1, 
			miniMap_Width, miniMap_Height, 
			hdcMiniMap, 
			0, 0, 
			SRCCOPY);
	...
	{		
		...
		int y = (*itr)->GetY()/16;*/
		int x = (*itr)->GetX()/16;
		int y = GAME_HEIGHT-miniMap_Height + (*itr)->GetY()/16;
