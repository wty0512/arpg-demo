...
int GameWorld::Init(HWND hwnd)
{
	...
	::GetCurrentDirectory(64, szDir);//Loading the current directory in a buffer
	::GetPrivateProfileString("server","IP","",szIP,16,(szDir+string("/config.ini")).c_str());//read ip from config.ini
	::GetPrivateProfileString("server","port","",szPort,16,(szDir+string("/config.ini")).c_str());//read port from config.ini
